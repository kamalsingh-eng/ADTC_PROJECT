...
WiFiClient espClient1321022;
PubSubClient client(espClient1321022);
long lastMsg = 0;
int first_scan=0;
...
client.setServer(serverHostname,mqttPort);
 ...
 // client.setServer(serverIp, mqttPort);
  ...
  while (!client.connected()) {
    ...
    if (client.connect("ESP8266Client1321022", mqttUser, mqttPassword )) {
...
void loop() {
  ...
  while (!client.connected())   {
    ...
    Serial.println("Connecting to MQTT...or wifi not connecting");
     digitalWrite(mqtt_pin, LOW);
    client.connect("ESP8266Client1321022", mqttUser, mqttPassword);
 ...
 Serial.println(distance3);
   ...
   client.publish("GRADIENT_DATA", JSONmessageBuffer);
   }
    client.loop();
