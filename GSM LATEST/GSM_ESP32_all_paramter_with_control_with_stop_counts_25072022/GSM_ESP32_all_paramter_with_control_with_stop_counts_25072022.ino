...
if (gps.location.isValid()) {
 ...
 float longitude = (gps.location.lng());  
    ...
    anArray1[arrayIndex] = longitude*1000000;
    //float longitude = (gps.location.lng()); 
    

    if ((difference > 25) &&(difference1 > 25) && f_flag == 1) {

      f_flag = 0 ;
      r_flag = 1;
      f_count = f_count +1 ;
    }
    if ((difference < -25) && (difference1 < -25) && r_flag == 1) {

      f_flag = 1 ;
      r_flag = 0;
      r_count = r_count +1 ;
    }
    if ((difference > 25) && (difference1 > 25)) {
     stop_flag = 1;
    }
    if ((difference < -25) && (difference1 < -25)){
   ...
   if ((stop_count_flag > 10) && (stop_flag == 1)){
    stop_count++; 
    stop_flag = 0 ;
   }
    if (arrayIndex == 6){
      difference = anArray[6] - anArray[1];
       difference1 = anArray1[6] - anArray1[1];
      arrayIndex = 0;
      memset(anArray, 0, sizeof(anArray));
      memset(anArray1, 0, sizeof(anArray));
    ...
    Serial.print("reverse_count:");
     ...
     Serial.println(stop_count);
       Serial.print("stop_count_flag:");
     Serial.println(stop_count_flag);
...
if (pub == 1 ){
    ...
    JSONencoder["NCH8"] = stop_count;
    JSONencoder["NCH9"] = stop_count_flag;
    JSONencoder["NCH10"] = stop_flag;
    JSONencoder["NCH11"]= difference1;

    JSONencoder1["name"] = "2002";
    ...
    Serial.println(JSONmessageBuffer);
    delay(100);
    client.publish("2002", JSONmessageBuffer1);
